...
public interface AuthApi {
    ...
    @Operation(operationId = "register", summary = "Register User",
            description = "Creates a new user account with the provided details. On success, the API returns the newly created user")
    @ApiResponses({
            @ApiResponse(responseCode = "400", description = "The request is invalid. One or more validation errors were found",
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "400", description = "The request is invalid. One or more validation errors were found",
                    content = @Content(mediaType = "application/problem+json", schema = @Schema(implementation = ProblemDetail.class))),
            @ApiResponse(responseCode = "401", description = "The provided credentials are invalid",
                    content = @Content(mediaType = "application/problem+json", schema = @Schema(implementation = ProblemDetail.class))),
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "400", description = "The request is invalid or the reset token has expired",
                    content = @Content(mediaType = "application/problem+json", schema = @Schema(implementation = ProblemDetail.class))),
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "401", description = "The refresh token is missing, invalid, or expired",
                    content = @Content(mediaType = "application/problem+json", schema = @Schema(implementation = ProblemDetail.class))),
